---
layout: post
title:  "T-Pot 16.03 - Enhanced Multi-Honeypot Platform"
date:   2016-03-11 03:37:00
...
[docker](https://www.docker.com/) and includes the dockerized honeypots
...
3. Enjoy your favorite beverage - [watch](http://sicherheitstacho.eu/?peers=communityPeers) and [analyze](#kibana).
...
- Thanks to Kibana 4.x SSH port forwarding can utilize any user defined local port `ssh -p 64295 -l tsec -N -L4711:127.0.0.1:64296 <yourHoneypotIPaddress`.
...
- The **`/usr/bin/backup_elk.sh`** allows to backup all ElasticSearch indexes including `.kibana` and `logstash` which contain all information to restore your data a freshly installed machine simply by entering `tar xvfz <backup-name>.tgz -C /`.
...
- During the creation of T-Pot 16.03 a total amount of 203 liters of [Club Mate](https://de.wikipedia.org/wiki/Club-Mate) was consumed :) 
...
[suricata](http://suricata-ids.org/) a Network Security Monitoring engine and the
[ELK stack](https://www.elastic.co/videos) to beautifully visualize all the events captured by T-Pot. Events will be correlated by our own data submission tool [ewsposter](https://github.com/dtag-dev-sec/ews) which also supports hpfeeds honeypot data sharing.
...
You can download the prebuilt installation image [here](http://community-honeypot.de/tpotce.iso) and jump to the [installation](#vm) section. The ISO image is hosted by our friends from [Strato](http://www.strato.de) / [Cronon](http://www.cronon.de).
    ...
    shasum tpotce.iso
    7d6819c75fe93e5df89713a8e917eddf53e15c43 tpotce.iso
